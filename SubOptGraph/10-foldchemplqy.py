...
def create_dataset(cfg): 
    ...
    root = './data/chemplqy'  
    train_dataset = Emitter(root, subset=0, split='train', transform=transform,file_name='origin_plqy.txt',)
    val_dataset = Emitter(root, subset=0, split='val', transform=transform_eval,file_name='origin_plqy.txt',) 
...
if __name__ == '__main__':
    # get config 
    cfg.merge_from_file('./configs/plqy.yaml')
    cfg = update_cfg(cfg)
    qy = [line.strip().split('\t') for line in open('./data/chemplqy/raw/origin_plqy.txt').readlines()]
    y = [float(i[2]) for i in qy]
