...
from Mol2Graph_bodipy import Mol2Graph
...
if __name__ == '__main__':
    ...
    cfg = update_cfg(cfg)
    emission = [line.strip().split('\t') for line in open('./data/bodipy/raw/origin_absorption.txt').readlines()]
    y = [float(i[2]) for i in emission]
    Y = np.array([i for i in y])
    mean = Y.mean()
    std = Y.std()
    snapshot_path = cfg.model.path
    run_k_fold(cfg, create_dataset, create_model, train, test,snapshot_path=snapshot_path,evaluator=None,mean=float(mean),std=float(std),k=5) 
