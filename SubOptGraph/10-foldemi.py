...
def create_dataset(cfg): 
    ...
    root = './data/emission'  
    train_dataset = Emitter(root, subset=0, split='train', transform=transform,file_name='origin_emission.txt',)
    val_dataset = Emitter(root, subset=0, split='val', transform=transform_eval,file_name='origin_emission.txt',) 
...
if __name__ == '__main__':
    # get config 
    cfg.merge_from_file('./configs/emission.yaml')
    cfg = update_cfg(cfg)
    emission = [line.strip().split('\t') for line in open('./data/emission/raw/origin_emission.txt').readlines()]
