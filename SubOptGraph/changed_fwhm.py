...
class Emitter(InMemoryDataset):
    ...
    def raw_file_names(self):
        if self.file_name == None:
            return 'origin_fwhm.txt'  # 针对不同的预测性质，修改源头数据文件名称
        else:
        ...
        if os.path.exists(osp.join(self.raw_dir,f'data_list.pt')):
            ...
            for idx in range(number):
                ...
                data = Data(x=torch.tensor(d.x,dtype=torch.float32),
                        ...
                        edge_attr=torch.tensor(d.edge_feats,dtype=torch.float32),
...
def create_dataset(cfg): 
    ...
    root = './data/fwhm'  
    train_dataset = Emitter(root, subset=0, split='train', transform=transform,percentage=0.80)
    val_dataset = Emitter(root, subset=0, split='val', transform=transform_eval,percentage=0.80) 
    test_dataset = Emitter(root, subset=0, split='test', transform=transform_eval,percentage=0.80)    
...
if __name__ == '__main__':
    # get config 
    cfg.merge_from_file('/public/home/xmpu220/emission/GNNAK/GNNAK/train/configs/fwhm.yaml')
    cfg = update_cfg(cfg)
    fwhm = [line.strip().split('\t') for line in open('/public/home/xmpu220/emission/GNNAK/GNNAK/train/data/0.80fwhm/raw/origin_fwhm.txt').readlines()]
