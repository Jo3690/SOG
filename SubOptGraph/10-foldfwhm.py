        ...
        if os.path.exists(osp.join(self.raw_dir,f'data_list.pt')):
            ...
            for idx in range(number):
                ...
                data = Data(x=torch.tensor(d.x,dtype=torch.float32),
                        ...
                        y=torch.tensor((float(items[idx][-1])-mean)/std, dtype=torch.float32),
       ...
       # for self.split in ['train', 'val', 'test']:
        if self.test == False:
            if self.split == 'train':
                ...
                data_list = data_list[int(number*self.percent):int(number*(self.percent+0.10))]

            elif self.split == 'test':
                if self.percent == 0.90:
                    data_list = data_list[int(number*0.90):]
                else:
                    data_list = data_list[int(number*(self.percent+0.10)):]
...
def create_dataset(cfg): 
    ...
    root = './data/fwhm'  
    train_dataset = Emitter(root, subset=0, split='train', transform=transform,file_name='origin_fwhm.txt',)
    val_dataset = Emitter(root, subset=0, split='val', transform=transform_eval,file_name='origin_fwhm.txt',) 
    ...
    return train_dataset, transform, transform_eval
...
if __name__ == '__main__':
    # get config 
    cfg.merge_from_file('./configs/fwhm.yaml')
    cfg = update_cfg(cfg)
    fwhm = [line.strip().split('\t') for line in open('./data/fwhm/raw/origin_fwhm.txt').readlines()]
    y = [float(i[2]) for i in fwhm]
    Y = np.array([i for i in y])
    mean = Y.mean()
    std = Y.std()
    snapshot_path = cfg.model.path
    run_k_fold(cfg, create_dataset, create_model, train, test,snapshot_path=snapshot_path,evaluator=None,mean=float(mean),std=float(std),k=10) 
