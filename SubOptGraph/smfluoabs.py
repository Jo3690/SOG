...
from Mol2Graph_smf import Mol2Graph
...
class Emitter(InMemoryDataset):
    ...
    def Graph(self,mol1,mol2):
        ...
        end_index = []  
        start_index = []
       # print(atom_index.shape,edge_index.shape)
        for i in atom_index:
...
def create_dataset(cfg): 
    ...
    root = './data/smfluoabs'  
    train_dataset = Emitter(root, subset=0, split='train', transform=transform,percentage=0.90)
    val_dataset = Emitter(root, subset=0, split='val', transform=transform_eval,percentage=0.90) 
...
def create_model(cfg):
    model = GNNAsKernel(nfeat_node=39, nfeat_edge=6, # node feature dimention changes with the atom types
...
if __name__ == '__main__':
    # get config 
    cfg.merge_from_file('./configs/absorption.yaml')
    cfg = update_cfg(cfg)
    emission = [line.strip().split('\t') for line in open('./data/smfluoabs/raw/origin_absorption.txt').readlines()]
