...
class TADF(InMemoryDataset):
    ...
    def __init__(self, root, subset=False, split='train', transform=None,
                 pre_transform=None, pre_filter=None,file_name=None,test=False,percentage=0.80,mean=0.,std=1.):
        self.subset = subset
        ...
        if os.path.exists(osp.join(self.raw_dir,f'data_list.pt')):
            ...
            for idx in range(number):
                ...
                data = Data(x=torch.tensor(d.x,dtype=torch.float32),
                        ...
                        y=torch.tensor((float(items[idx][2])-self.mean)/self.std, dtype=torch.float32),
