...
def set_cfg(cfg):
    ...
    cfg.device = 0#torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    cfg.model.gnn_type = 'TransformerConv' # yoru can change the GNN type according to pyg_gnn_wrapper package to extract subgraph features with different GNN
    # Hidden size of the model
    cfg.model.hidden_size = 128
    # Number of gnn layers (doesn't include #MLPs)
    cfg.model.num_layers = 5
    # Number of inner layers, 0 means use normal GNN
    cfg.model.mini_layers = 2
    ...
    cfg.model.use_normal_gnn = True
    # dim for global state
    cfg.model.state_dim = 5
    # file path for model paras
    cfg.model.path = "./train/Emi_paras/"
    #for file name
    cfg.model.file_name = 'test.txt'
    # ------------------------------------------------------------------------ #
